...
userRoutes.get('/variables', [autenticacion_1.verificaToken], (req, resp) => {
    ...
    try {
        ...
        console.log(colors_1.default.bgRed('**********************ERROR getdevicecredentials**********************'.bgRed));
        console.log(error);
        resp.json({
            ok: false,
            usuario,
...
//FUNCTION para el descencriptar
function descencriptarTexto(encryptedText, vector_iv, secretKey) {
    const iv = crypto_js_1.default.enc.Hex.parse(vector_iv);
    const decifrado = crypto_js_1.default.AES.decrypt(encryptedText, secretKey, {
        iv: iv,
        mode: crypto_js_1.default.mode.CBC,
        padding: crypto_js_1.default.pad.Pkcs7
    });
    return decifrado.toString(crypto_js_1.default.enc.Utf8);
}
